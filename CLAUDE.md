...
- **Components** (`src/components/`): `Avatar` (colored initials), `TripCard`, `MapSidebar` (feed map), `TripStepMap` (SVG polyline route + active/first/last dots), `Step` (a step row — collapsed by default in feeds; an inline "Comment" toggle button expands it and lazily fetches the Bluesky thread, so the same component is reused, just pre-expanded, by `StepDetailPage`), `ImageCarousel` (Instagram-style carousel for a step's photos), `SideNav`, `AddTripModal`/`AddStepModal` (creation forms, posting AT Proto records via `UserTripContext`).
...
- Fetching that data (`agent.getPostThread`) is expensive and rate-limited, so `Step.tsx` fetches it lazily: a step row is collapsed by default (content only, no thread call) and only loads likes/comments when its "Comment" toggle button is expanded. `/profile/:handle/step/:rkey` (`StepDetailPage.tsx`) is a permalink that renders the same `Step` component pre-expanded (`defaultExpanded`) — there's no separate detail layout or duplicated thread-fetching logic.
...
- Step photos work the same way: `AddStepModal` uploads each file via `agent.uploadBlob` (max 4, Bluesky's own embed limit) and `UserTripContext.createStep` stores the resulting `BlobRef`s both on the step record (`StepRecord.photos`) and as the cross-post's `app.bsky.embed.images`. To display them, `Step`/`ImageCarousel` don't fetch anything — they parse the author DID straight out of `step.uri` (`at://<did>/collection/rkey`) and build `https://cdn.bsky.app/img/feed_fullsize/plain/{did}/{cid}@jpeg` URLs from each photo's blob CID. This only resolves for images that were actually cross-posted (i.e. indexed by the Bluesky network), which is guaranteed here since every step is cross-posted at creation time.
...
- Keep likes/comments Bluesky-backed per the Social layer section above — don't introduce our own comment/like records, and only fetch a post thread from `Step` (collapsed feed row or `StepDetailPage`'s pre-expanded one), never elsewhere.
